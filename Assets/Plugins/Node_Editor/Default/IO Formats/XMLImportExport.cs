...
namespace NodeEditorFramework.IO
{
    public class XMLImportExport : StructuredImportExportFormat
    {
        ...
        public override CanvasData ImportData(params object[] args)
        {
            ...
            using (FileStream fs = new FileStream(path, FileMode.Open))
            {
                ...
                foreach (XmlElement xmlEditorState in xmlEditorStates)
                {
                    ...
                    float zoom;
                    if (!TryParseFloat(xmlEditorState.GetAttribute("zoom"), out zoom))
                        zoom = 1;
                    // Selected Node
        ...
        {
            float result = 0;
            if (!TryParseFloat(element.GetAttribute(attribute), out result) && throwIfInvalid)
        ...
        private Vector2 GetVectorAttribute(XmlElement element, string attribute, bool throwIfInvalid = false)
        {
            ...
            float vecX, vecY;
            if (vecString.Length == 2 && TryParseFloat(vecString[0], out vecX) && TryParseFloat(vecString[1], out vecY))
        ...
        private Color GetColorAttribute(XmlElement element, string attribute, bool throwIfInvalid = false)
        {
            ...
            if (vecString.Length == 4 && TryParseFloat(vecString[0], out colR) && TryParseFloat(vecString[1], out colG) && TryParseFloat(vecString[2], out colB) && TryParseFloat(vecString[3], out colA))
        ...
        private Rect GetRectAttribute(XmlElement element, string attribute, bool throwIfInvalid = false)
        {
            ...
            float x, y, w, h;
            if (vecString.Length == 4 && TryParseFloat(vecString[0], out x) && TryParseFloat(vecString[1], out y) && TryParseFloat(vecString[2], out w) && TryParseFloat(vecString[3], out h))
