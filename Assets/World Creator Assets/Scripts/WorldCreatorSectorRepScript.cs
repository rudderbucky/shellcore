...
public class WorldCreatorSectorRepScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        transform.localScale = rend.bounds.size;
        var grad = 5.9F / (WorldCreatorCamera.maxZ - WorldCreatorCamera.minZ);
        if (WorldCreatorCamera.instance)
        {
            rend.startWidth = rend.endWidth = 6F - (grad * WorldCreatorCamera.maxZ) + grad * WorldCreatorCamera.instance.transform.position.z;
