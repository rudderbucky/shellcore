...
public class WorldCreatorSectorRepScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        transform.localScale = rend.bounds.size;
        var grad = 0.9F / (WorldCreatorCamera.maxZ - WorldCreatorCamera.minZ);
        if (WorldCreatorCamera.instance)
        {
            rend.startWidth = rend.endWidth = 1F - (grad * WorldCreatorCamera.maxZ) + grad * WorldCreatorCamera.instance.transform.position.z;
