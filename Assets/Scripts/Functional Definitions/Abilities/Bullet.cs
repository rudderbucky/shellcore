...
public class Bullet : WeaponAbility
{
    ...
    protected virtual void FireBullet(Vector3 targetPos)
    {
        ...
        if(targetingSystem.GetTarget()) predictionAdjuster = (targetingSystem.GetTarget().GetComponentInChildren<Rigidbody2D>().velocity
            - Core.GetComponent<Rigidbody2D>().velocity) * survivalTime 
            / (bulletSpeed / 2);
