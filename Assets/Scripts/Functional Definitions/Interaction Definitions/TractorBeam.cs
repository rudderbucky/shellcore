...
public class TractorBeam : MonoBehaviour
{
    ...
    public float tractorStrength = 1f;
            ...
            if (rigidbody)
            {
                ...
                else if (dist > 2f)
                {
                    if(!owner.tractorSwitched)
                        rigidbody.AddForce(dir.normalized * (dist - 2F) * (rigidbody.mass / Time.fixedDeltaTime) * tractorStrength);
                    else
                        owner.GetComponent<Rigidbody2D>().AddForce(-dir.normalized * (dist - 2F) * (rigidbody.mass / Time.fixedDeltaTime) * tractorStrength);
                }
            }
        }
    }
